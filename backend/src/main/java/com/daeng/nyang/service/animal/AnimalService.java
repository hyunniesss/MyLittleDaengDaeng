...
public class AnimalService {
	...
	public List<AnimalListFE> allAnimalRead() {
		// 1. 동물리스트
		List<Animal> animalList = animalRepo.findAll();	// AnimalRepo에서 findAll 메소드 실행
		List<Ptag> personalityList = ptagRepo.findAll(); // PtagRepo에서 findAll 메소드 실행
		List<AnimalListFE> result = new ArrayList<>();	// 위 두 메소드 실행에서 필요한 정보를 골라서 담을 예정 -> 프론트에 전달할 데이터

		for (int i = 0; i < animalList.size(); i++) {
			Animal animal = animalList.get(i);
			// 2. 각 동물의 personality
			List<String> personalList = new ArrayList<>();
			for (int j = 0; j < personalityList.size(); j++) { // ptag에서 가져온 personalityList에서 desertion_no가 동물정보와 맞는 tagname들만 리스트에 저장 
			...
			);  // @Builder를 이용해서 AnimalListFE 객체 하나씩 만들어서 반환할 데이터에 담음
